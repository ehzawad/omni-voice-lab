...
NON_BN_LETTER = re.compile(r"[一-鿿぀-ヿЀ-ӿ؀-ۿऀ-ॣ०-ॿ]")
...
def script_ok(reply):
    ...
    letters = re.findall(r"\w", reply)
    if not letters:
        return False, 0.0
    bn = len(BN.findall(reply))
    frac = bn / max(1, len(letters))
...
def run_text(scen, out):
    ...
    for sc in scen["scenarios"]:
        ...
        for i, turn in enumerate(sc["turns"]):
            ...
            t0 = time.time()
            gen, spoken = eng.respond(msgs, threading.Event(), lambda d: None, lambda s: None, lambda p: None)
...
async def run_audio(scen, out, token, pause_s=1.2):
    ...
    for sc in scen["scenarios"]:
        ...
        ev_log, state = [], {"turn_end": 0, "asr": [], "reply": [], "first": [], "cur_reply": ""}
        async with websockets.connect(f"ws://127.0.0.1:{C.GW_PORT}/ws", max_size=None) as ws:
            ...
            async def reader():
                async for m in ws:
                    if isinstance(m, str):
                        ...
                        if t == "asr": state["asr"].append(d.get("text", ""))
                        elif t == "text": state["cur_reply"] += d.get("delta", "")
                        elif t == "metrics" and d.get("first_audio_ms"): state["first"].append(d["first_audio_ms"])
                        elif t == "turn_end":
                            state["reply"].append(state["cur_reply"]); state["cur_reply"] = ""
                            state["turn_end"] += 1
            ...
            for i, turn in enumerate(sc["turns"]):
                ...
                asr_txt = state["asr"][i] if i < len(state["asr"]) else ""
                reply = state["reply"][i].strip() if i < len(state["reply"]) else ""
                first = state["first"][i] if i < len(state["first"]) else None
                kw = keyword_hit(reply, turn["expect"]); sc_ok, frac = script_ok(reply)
                ...
                print(f"  t{i} {flag} asr_cer={cer:.2f} kw={'Y' if kw else 'n'} judge={jv:3s} "
                      f"first_audio={first or 0:5.0f}ms | heard: {asr_txt[:34]}")
                print(f"        A: {reply[:110]}")
...
def summarize(results, mode):
    ...
    if mode == "audio":
        ...
        if fa: print(f"  first audio: median {np.median(fa):.0f} ms, p90 {np.percentile(fa,90):.0f} ms")
